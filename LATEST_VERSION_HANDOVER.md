...
- Confirmed the v0.9.4 enemy-animation source at `28fa8e5`, merged it into
  `main` at `4d8758d`, pushed `main`, and independently refetched the remote ref.
- Replaced all 32 player-attack runtime strips with separate 2048x1024 RGBA
  sheets containing eight distinct 512x512 animation frames per stable ID.
- Kept combat timing, RNG, range growth, damage, collision, and cooldowns
  unchanged; added deterministic visual-only offsets and native-or-smaller,
  uniform rendering with bounded long-beam visuals.
- Added tiny boxed `v0.9.5` tags only to the title and pause menus.
- Passed 419 tests, zero lint findings across 192 files, source version sync,
  zero-risk media audit, full runtime/source byte parity, and preliminary
  package integrity. Clean-commit packaging and boot verification follow.

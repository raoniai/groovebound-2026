...
T["active campaign promotes Continue Campaign to the first primary action"] = function()
  with_dimensions(function()
    ...
    screen:_layout()
    H.eq(#screen.button_list.buttons, 6)
    H.eq(screen.button_list.buttons[1].label, "CONTINUE CAMPAIGN")
    H.eq(screen.button_list.buttons[1].variant, "primary")
    H.eq(screen.button_list.buttons[2].label, "NEW GAME")
    H.eq(screen.button_list.buttons[3].label, "WORLD TOUR CATALOG")
    H.eq(screen.button_list.buttons[6].label, "RESET CAMPAIGN")
    H.eq(screen.button_list.buttons[6].variant, "danger")
